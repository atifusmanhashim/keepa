...
function download_external_image($url, $post_id, $req_image_name) {
   // $image_name = 'product-image-' . $image_number . '.jpg'; // Change the image name as needed
    $image_name = $req_image_name . '.jpg'; // Change the image name as needed
...
function insert_product($product_data)
{
		...
		if ($product_id) {
			...
			if (is_a($product, 'WC_Product')) {
				...
				$req_image_name=$product_img;
				$attachment_id = download_external_image($image_url, $product_id,$req_image_name);
				print_r($attachment_id);
				set_featured_image($product_id, $attachment_id);
			} else {
				echo 'Product is not a valid WooCommerce product.';
			}
		} else {
...
function get_deals_products()
{
    ...
    try {
       ...
       // $deals_products=array_slice($deals_products, 0, 5);
