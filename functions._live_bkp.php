...
require_once get_stylesheet_directory() . "/keepa/keepa.php";
...
function my_theme_enqueue_styles() {
    wp_enqueue_style( 'matt-style', get_template_directory_uri() . '/style.css' );
...
function download_external_image($url, $post_id, $image_number,$req_image_name) {
   // $image_name = 'product-image-' . $image_number . '.jpg'; // Change the image name as needed
    $image_name = $req_image_name . '-' . $image_number . '.jpg'; // Change the image name as needed
...
function insert_product($product_data)
{
    	...
    	$product_category_name = $product_data->product_category_name; // Replace with your desired category name
        $product_category_slug = $product_data->product_category_slug; // Replace with your desired category slug
        $image_url=$product_data->product_img_url;
        $product_img=$product_data->product_img;
        
    	if (!term_exists($product_category_name, 'product_cat')) {
            $term_category = wp_insert_term(
                $product_category_name, // Category name
                'product_cat',  // Taxonomy (product_cat for WooCommerce product categories)
                array(
        ...
        $product = wc_get_product( get_page_by_title( $product_sku, OBJECT, 'product' ) );
        if ( $product ) {
           ...
           // Get the product object
			...
			$product_obj->set_regular_price($product_current_price); // in current shop currency
			$product_obj->set_slug($product_sku);
    		$product_obj->set_sku($product_sku); // Should be unique
            
     		$product_obj->set_short_description($product_description);
     		// you can also add a full product description
     		$product_obj->set_description($product_title);
    	...
    	wp_set_object_terms($product_id, array($category_id), 'product_cat');
        $image_number=1;
        $req_image_name=$product_sku;
        $attachment_id = download_external_image($image_url, $product_id, $image_number,$req_image_name);
        print_r($attachment_id);
        set_featured_image($product_id, $attachment_id);
}
...
function get_deals_products()
{
    ...
    try {
        ...
        //$deals_products=array_slice($deals_products, 0, 40);
        ...
        foreach($deals_products as $product)
        {
            ...
            try {
                ...
                $product_img_url="https://m.media-amazon.com/images/I/".explode(',', $prod_data->imagesCSV)[1]."";
                $product_img_file=explode(',', $prod_data->imagesCSV)[1];
...
//add_action('init', 'get_deals_products');
add_action('import_keepa_products_cron', 'get_deals_products');
